...
public class KubernetesTaskExecutorRunner {
    ...
    private static void runTaskManagerSecurely(String[] args) {
        ...
        // Load the Task Native Execution Library (libtnel.so and its dependencies) and initialize
        // the native runtime. The Kubernetes entry point does not go through TaskManagerRunner.main,
        // so we replicate the native bootstrap here to keep the K8s path consistent with the
        // standalone/YARN path.
        TNELLibrary.loadLibrary(args);
        TaskManagerRunner.initTMConfiguration(checkNotNull(configuration));
